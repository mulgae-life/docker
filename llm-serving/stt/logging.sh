#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# logging.sh — STT(vLLM) 호출 로그를 5분마다 S3로 동기화 (호출 카운트 통계용)
#
# vllm/logging.sh 와 동일한 sync 로직을 사용한다 (start.sh 패턴 풀 도입).
# 본 스크립트는 stt/ 디렉토리에서 동작하며 SRC 로그 파일/S3 prefix만 STT용이다.
#
# 사용법:
...
#   호출 수: aws s3 cp s3://hgi-ai-res/logs/stt/voxtral/$(date +%F).log - | wc -l
#
# 일자 분할:
...
# 인자 1개가 있으면 인스턴스명으로 사용 ('stop' 제외). 그 외는 기본값 'voxtral'.
INST="voxtral"
[ "${1:-}" != "stop" ] && [ -n "${1:-}" ] && INST="$1"

S3_BUCKET="${S3_BUCKET:-hgi-ai-res}"
S3_PREFIX="${S3_PREFIX:-logs/stt}"
