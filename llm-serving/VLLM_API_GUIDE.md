...
> 🔒 **PII/DLP 가드 (필독)** — `:5015` 진입점은 사내 정책에 따라 **개인정보 가드**를 거칩니다. 호출 주소·방식은 그대로지만, 요청(in)과 응답(out)의 텍스트가 모두 검사됩니다.
...
> - **스트리밍**: 응답 PII 검사를 위해 현재 `stream:true`라도 완결 후 한 번에 전달됩니다 (토큰 점진 출력 일시 비활성 — [§3.1](#31-스트리밍-sse)).
> - **이미지 입력**: 멀티모달(이미지) 콘텐츠는 현재 PII 검사 범위 밖입니다 (텍스트만 검사 — [§3.3](#33-이미지-멀티모달-vision)).
> - **장애 시**: PII 엔진 일시 장애면 보안상 요청을 차단합니다 (HTTP 503 `pii_unavailable`, fail-closed).
...
> 🔒 **PII 가드 영향 (현재 제약)**: `:5015`는 응답 PII 마스킹을 보장하기 위해 스트리밍을 **완결 후 1회 방출**합니다. 즉 `stream:true`를 줘도 토큰이 점진적으로 오지 않고, 마스킹이 끝난 전체 응답이 단일 청크로 도착합니다(`data: [DONE]` 형식·파싱 코드는 동일하게 동작). 토큰 점진 출력이 필요하면 운영자에게 buffer 모드 활성화 여부를 문의하세요(후속 예정).
...
> 🔒 **PII 가드**: 이미지(멀티모달) 콘텐츠는 현재 PII 검사 대상이 **아닙니다**. 텍스트 메시지만 in/out 검사됩니다. 개인정보가 담긴 문서 이미지를 보낼 때는 이 점에 유의하세요.
