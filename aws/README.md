...
| `setup-ec2.sh: 디바이스 또는 자식 파티션이 ... 마운트됨` 에러 | 루트/시스템 디스크 또는 자식 파티션이 이미 사용 중. `lsblk`로 `/`나 `/boot/efi`가 안 붙은 추가 EBS만 지정 (디스크 전체 `/dev/nvme0n1` 입력 시에도 자식 `p1`이 `/`에 붙어 있으면 차단됨) |
