...
| `LLM_CODE_SERVER_PORT` | code-server 포트 (기본 `5500`, `LLM_EXTRA_PORTS`와 겹치지 않게) | — |
...
sudo ~/aws/user.sh up service-a --root --password aiteam12 --service-port 5031 --code-port 5510 --gpus 0,1
sudo ~/aws/user.sh up service-b --root --password aiteam12 --service-port 5015 --code-port 5520 --gpus 2,3
...
| 컨테이너 | code-server 포트 |
|----------|:----------------:|
| 메인 (`docker compose`) | `LLM_CODE_SERVER_PORT` (기본 5500) |
| `user.sh` 일반 사용자 | 각 포트 범위의 첫 포트 (5011 / 5021 / …) — 자동 할당 |
| `user.sh --root` 다중 운용 | `--code-port` 인자 또는 `.env LLM_CODE_SERVER_PORT` 폴백 |
...
| `docker compose build` 시 `vsix/` 디렉토리 없음 에러 | `Dockerfile.llm`이 `COPY vsix/`를 수행. 폴더 자체는 필수(파일은 비어 있어도 OK). `mkdir -p ~/aws/vsix` 후 재빌드 |
