...
| 우선순위 | 작업 | 상태 |
|---------|------|------|
| P1 | **`gemma-4` 별칭 운영계 반영**: 8/20 연구계 적용·검증 완료. 잔존 — `./start.sh push` 후 운영계에서 인스턴스·게이트웨이 **둘 다** 재기동(게이트웨이만 하면 호환 계층은 붙고 별칭은 옛 이름). 클라이언트가 보낼 `model` 값은 `VLLM_API_GUIDE.md` §5에 있다. | 연구계 ✅, 운영계 대기 |
